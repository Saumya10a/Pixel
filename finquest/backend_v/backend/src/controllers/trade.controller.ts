...
export async function listTradesController(req: any, res: any) {
  const trades = await Trade.find({ user: req.user!.id }).sort({ createdAt: -1 })
  return res.json(new ApiResponse("Trades", trades))
}

export async function createTradeController(req: any, res: any) {
  const { symbol, side, price, qty } = req.body as z.infer<typeof createTradeSchema>
