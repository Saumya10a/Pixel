...
export async function listLessonsController(req, res) {
...
export async function updateProgressController(req, res) {
    ...
    const doc = await LessonProgress.findOneAndUpdate({ user: req.user.id, lesson: lessonId }, { $max: { percent } }, // only increase progress
    ...
    return res.json(new ApiResponse("Progress updated", {
        lessonId,
        percent: doc.percent,
        xpAdded,
